...
// ** Reactstrap Imports
import { Form, Label, Row, Col, Button, Input, FormFeedback } from "reactstrap";
...
const FormRegular = (props) => {
  ...
  const forms = useForm({
    defaultValues: {
      ...
      penerimaManfaatOther: formData?.penerimaManfaatOther || "",
      jumlahUnit: formData?.jumlahUnit || 0,
  ...
  const penerimaManfaatId = watch("penerimaManfaat");
    ...
    <FormProvider {...forms}>
      <Form onSubmit={handleSubmit(onSubmit)}>
        <Row>
          ...
          {Number(penerimaManfaatId) === 9999 && (
            <Col md="6" className="mb-1">
              <Label className="form-label" for="penerimaManfaatOther">
                Keterangan Penerima Manfaat
              </Label>
              <Controller
                id="penerimaManfaatOther"
                name="penerimaManfaatOther"
                control={control}
                render={({ field }) => (
                  <Input
                    placeholder="cth: Kerumunan Warga Desa"
                    invalid={errors.penerimaManfaatOther && true}
                    {...field}
                  />
                )}
              />
              {errors.penerimaManfaatOther && (
                <FormFeedback>
                  {errors.penerimaManfaatOther.message}
                </FormFeedback>
              )}
            </Col>
          )}
